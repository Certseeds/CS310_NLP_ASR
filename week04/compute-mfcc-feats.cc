 ...
 * @LastEditTime: 2020-03-13 21:13:37
 */
...
int main(int argc, char *argv[]) {
  try {
    ...
    // Register the MFCC option struct.//其自己处理命令行参数
    mfcc_opts.Register(&po);
    // 此处注册的全都是带--的命令行参数,
    // 50 - 66 Register the options.
    ...
    // 这几个都是kaldi内置的读取器,最终是utt to float.
    ...
    for (; !reader.Done(); reader.Next()) {
      ...
      // 又一个读取器,从wave_data中读取data的 channel
      SubVector<BaseFloat> waveform(wave_data.Data(), this_chan);
      Matrix<BaseFloat> features;
      try {
        // kaldi\src\feat\feature-common-inl.h
